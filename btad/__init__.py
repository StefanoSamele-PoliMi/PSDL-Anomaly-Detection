...
class BTADataset(Dataset):
    def __init__(self, dataset_path, class_name='01', is_train=True, resize=256, cropsize=224):
    ...
    def load_dataset_folder(self):
        ...
        for img_type in img_types:
            ...
            if img_type == 'ok':
                ...
                gt_files_list = os.listdir(gt_type_dir)
                gt_file_type = '.' + gt_files_list[0].split('.')[1]
                gt_fpath_list = [os.path.join(gt_type_dir, img_fname + gt_file_type)
    ...
    def load_all_dataset(self):
        ...
        for mvtec_class in os.listdir(self.dataset_path):
            ...
            for img_type in img_types:
                ...
                if not os.path.isdir(img_type_dir):
                    continue
                img_fpath_list = sorted([os.path.join(img_type_dir, f) for f in os.listdir(img_type_dir) if (f.endswith('.png') or f.endswith('.bmp'))])
                x.extend(img_fpath_list)
                # load gt labels
                if img_type == 'ok':
                    y.extend([0] * len(img_fpath_list))
